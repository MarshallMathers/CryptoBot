...
def initialize(context):
	...
	context.model.load_weights("dqn_trader_weights.h5f")
	context.lastTrade=0
...
#	start = time.time()
	...
	opens = data.history(context.asset,'open',bar_count=startMin,frequency='1T')
	volume = data.history(context.asset,'low',bar_count=startMin,frequency='1T')
	price = data.current(context.asset, 'price')
	
	inData=[[],[],[],[],[]]
	inData[0] = np.array(low)
	inData[1] = np.array(high)
	inData[2] = np.array(opens)
	inData[3] = np.array(close)
	inData[4] = np.array(volume)
	#5-6-7
	for i in range(3):
		inData.append(np.array(list(ta.momentum.rsi(close,n=((rsiN+(i*2))*csTime)))))
	#7-8-9
	for i in range(3):
		inData.append(np.array(list(ta.trend.ema_fast(close,n_fast=((emaFN+(i*2))*csTime)))))
		#inData[6] = np.array(list(ta.trend.ema_fast(close,n_fast=emaFN)))
	#10-11-12
	for i in range(3):
		inData.append(np.array(list(ta.trend.ema_slow(close,n_slow=((emaSN+(i*4))*csTime)))))
		#inData[7] = np.array(list(ta.trend.ema_slow(close,n_slow=emaSN)))
	#13-14-15
	for i in range(3):
		inData.append(np.array(list(ta.volatility.keltner_channel_hband(high, low, close, n=((keltN+(i*2))*csTime)))))
		#inData[8] = np.array(list(ta.volatility.keltner_channel_hband(high, low, close, n=keltN)))
	#16-17-18
	for i in range(3):
		inData.append(np.array(list(ta.volatility.keltner_channel_lband(high, low, close, n=((keltN+(i*2))*csTime)))))
		#inData[9] = np.array(list(ta.volatility.keltner_channel_lband(high, low, close, n=keltN)))
	#19-20-21
	for i in range(3):
		inData.append(np.array(list(ta.trend.macd(close, n_fast=((macdFN+(i*2))*csTime),n_slow=((macdSN+(i*4))*csTime)))))
		#nData[10] = np.array(list(ta.trend.macd(close, n_fast=((macdFn+(i*2))*csTime),n_slow=((macdSN+(i*4))*csTime))))
	NN = []
	for i in range(len(inData)):
		NN.append(inData[i][-1])
